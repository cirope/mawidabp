module Findings::FollowUpLastChanged
  ...
  def follow_up_date_last_changed_on_versions
    ...
    versions.map(&:reify).reject(&:blank?).reverse_each do |reify|
      if current.follow_up_date != reify.follow_up_date
        return I18n.l(current.updated_at, format: :minimal)
      end

      current = reify
    end

    current.follow_up_date.present? ? I18n.l(current.updated_at, format: :minimal) : nil
  end
end
