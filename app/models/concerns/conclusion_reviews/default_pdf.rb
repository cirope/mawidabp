module ConclusionReviews::DefaultPdf
  ...
  def default_pdf organization = nil, *args
    ...
    put_default_cover_on                   pdf, organization
    put_default_watermark_on               pdf
    put_default_header_on                  pdf
    put_default_conclusion_on              pdf, options
    put_default_weaknesses_brief_on        pdf, organization
    put_default_findings_on                pdf, :weaknesses, options
    put_default_findings_on                pdf, :oportunities, options
  ...
  private
    ...
    def put_default_conclusion_on pdf, options
      grouped_objectives = grouped_control_objectives options

      if options[:brief].blank?
        put_default_objective_and_scopes_on pdf, grouped_objectives, options
      else
    ...
    def put_default_weaknesses_brief_on pdf, organization
    ...
    def put_default_findings_on pdf, type, options
      ...
      if review_has_findings || (ordered_by_risk && type == :weaknesses)
        ...
        if ordered_by_risk
          put_default_findings_by_risk_on pdf, type, use_finals
        else
          put_default_control_objective_findings_on pdf, grouped_objectives, type, use_finals
        end
      end
    end
    ...
    def put_default_objective_and_scopes_on pdf, grouped_control_objectives, options
      ...
      if REVIEW_MANUAL_SCORE
        ...
        pdf.text "\n#{ConclusionReview.human_attribute_name('planned_risk')}: #{planned_risk_text}\n"
    ...
    def put_default_score_table_on pdf
    ...
    def put_default_score_text_on pdf
    ...
    def put_default_no_score_text_on pdf
    ...
    def put_default_findings_by_risk_on pdf, type, use_finals
      ...
      put_default_repeated_findings_by_risk_on pdf, repeated
      put_default_present_findings_by_risk_on  pdf, present
    end

    def put_default_repeated_findings_by_risk_on pdf, findings
      pdf.move_down (PDF_FONT_SIZE * 0.75).round
      pdf.add_title I18n.t('conclusion_review.repeated_findings'),
        (PDF_FONT_SIZE * 1.15).round

      if findings.any?
        put_default_findings_sorted_by_risk_on pdf, findings
      else
        pdf.move_down PDF_FONT_SIZE
    ...
    def put_default_present_findings_by_risk_on pdf, findings
    ...
    def put_default_findings_sorted_by_risk_on pdf, findings
      findings.sort_for_review.each do |finding|
        coi = finding.control_objective_item

        put_default_control_objective_table_on pdf, coi, coi.process_control
    ...
    def put_default_control_objective_findings_on pdf, grouped_control_objectives, type, use_finals
      grouped_control_objectives.each do |process_control, cois|
    ...
    def default_control_objective_column_data_for control_objective_item, process_control
      caption = ControlObjective.model_name.human
      data    = []

      data << default_finding_column_headers(process_control)
      data << ["<b>#{caption}:</b> #{control_objective_item.to_s}\n"]

      data
    end

    def default_finding_column_headers process_control
      [
