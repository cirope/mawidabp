...
class OrganizationRoleTest < ActiveSupport::TestCase
  ...
  test 'search' do
    fixture_organization_role = organization_roles(
      :admin_role_for_administrator_in_cirope)
  ...
  test 'create' do
    ...
    assert_equal users(:administrator).id,
  ...
  test 'included attributes' do
    @organization_role.sync_ad = nil

    assert @organization_role.invalid?
    assert_error @organization_role, :sync_ad, :inclusion
  end
end
