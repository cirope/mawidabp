...
class EmailReceiverStrategies::MGraphStrategyTest < ActionController::TestCase
  setup do
    @strategy                = EmailReceiverStrategies::MGraphStrategy.new
  ...
  test 'add answers in findings when receive email' do
    ...
    response_stub = [new_email('id test', supervisor.email, "subject test [##{finding.id}]", body)]

    @strategy.stub :all, response_stub do
      @strategy.stub :set_read_email, nil do
  ...
  test 'should return clean answer before regex' do
    ...
    assert_equal 'Reply ', @strategy.clean_answer(new_email('id test', 'email@test.com', 'subject', body))
  end

  test 'should return - when body is blank' do
    body = ''

    assert_equal '-', @strategy.clean_answer(new_email('id test', 'email@test.com', 'subject', body))
  end

  test 'should return same body when dont have regex' do
    body = 'Reply test'

    assert_equal body, @strategy.clean_answer(new_email('id test', 'email@test.com', 'subject', body))
  end

  private

    def new_email id, from, subject, body
